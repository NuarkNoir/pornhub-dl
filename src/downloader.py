import urllib.request, urllib.error, urllib.parse, os, time, shutil
class Download:
    ...
    def now(self):
        try:                        
            ...
            if(self.current_file_size>0):
                temp = urllib.request.urlopen(self.url).info()
                actual_file_size = int(temp["Content-Length"])
            ...
            header = req.info()
            actual_file_size = int(header["Content-Length"])
            ...
            while True:
                ...
                print(("\r[PROGRESS] : [%3.2f%%] [%3.2f MB] [%s%s] [%3.2fKbps]" %((already_downloaded_size*100./actual_file_size), dlMB, '#' * done, '.' * (50-done), (dlMB*1024/(time.clock() - clk_start)) ) ), end=' ')            
